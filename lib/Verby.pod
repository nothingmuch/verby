...
An installer has two inputs, which are combined.

The first is the user's configuration - the parameters to influence the
installation, and the second is the sequence to install.

L<Verby> provides concepts and a reusable implementation of the concepts to
ease quick development of the combination of the two.
...
command line app would have three config sources:
	...
	my $args = Config::Source::ARGV->new; # foo --key=value
	my $config_file = Config::Source::XML->new("config.xml");
	my $conifg_prompt = Config::Source::Prompt; # last resort

	my $config_hub = Config::Data->new($args, $config_file, $config_prompt);
